...
//var strMensajeCantidadMaximaSuperada = 'Supera la cantidad máxima';
//var isSumarCantidad = false;
//var NoPasoUnaVez = true;
...
function AgregarCarritoHtml(pIndexCarrito) {
    ...
    if (isNotNullEmpty(listaCarritos[pIndexCarrito].proximoHorarioEntrega)) {
...
function onClickBuscar() {
    ...
    if (varPalabraBuscador != '') {
        if (varPalabraBuscador.length > 2) {
            if ($('#checkNombre').is(':checked') || $('#checkBoxCodigoBarra').is(':checked') || $('#checkBoxMonodroga').is(':checked') || $('#checkBoxLaboratorio').is(':checked') || $('#checkBoxCodigoAlfaBeta').is(':checked') || $('#checkBoxTroquel').is(':checked')) {
                //
    ...
    if (args != null) {
        if (args != '') {
            ...
            if (listaProductosBuscados.length > 0) {
                ...
                strHtml += '<tr>';
                strHtml += '<th class="bp-off-top-left thOrdenar" rowspan="2" onclick="onclickOrdenarProducto(-1)"><div class="bp-top-left">Detalle Producto</div></th>';
                ...
                for (var i = 0; i < listaProductosBuscados.length; i++) {
                    ...
                    strHtml += '<span style="font-weight:normal !important;color:#444 !important;font-size:10px !important;text-align:left !important;"  OnMouseMove="OnMouseMoveProdructo(event)" OnMouseOver="OnMouseOverProdructo(' + i + ')" OnMouseOut="OnMouseOutProdructo()" onclick="RecuperarTransfer(' + i + ')" style="cursor:pointer;" >' + AgregarMark(listaProductosBuscados[i].pro_nombre);
                    // Agregar:
...
function DesmarcarFilaSeleccionada() {
    ...
    //        var nombreAnt = selectedInput.id;
    //        nombreAnt = nombreAnt.replace('inputSuc', '');
    //        var palabrasBaseAnt = nombreAnt.split("_");
    //        var filaAnt = parseInt(palabrasBaseAnt[0]);
    //        $('.cssFilaBuscador_' + filaAnt).removeClass('borderFilaBuscadorSeleccionada');        
    //    }
}
...
function onfocusSucursal(pValor) {
    // seleccionar fila seleccionada
    //  DesmarcarFilaSeleccionada();
    // MarcarFilaSeleccionada(pValor);
    // fin seleccionar fila seleccionada
...
function onblurSucursal(pValor) {
    ...
    var palabrasBase = nombre.split("_");
    var fila = parseInt(palabrasBase[0]);
    // $('.cssFilaBuscador_' + fila).attr('style', 'background-color: red !important');
...
function CargarOActualizarListaCarrito(pIdSucursal, pIdProduco, pCantidadProducto, pIsDesdeBuscador) {
    ...
    if (isCarritoDiferido) {
        ...
        PageMethods.ActualizarProductoCarrito(pIdProduco, pIdSucursal, pCantidadProducto, false, OnCallBackActualizarProductoCarrito, OnFail);
    }
}
...
//    LimpiarTextBoxProductosBuscados(listaCarritos[indexCarrito].codSucursal);
//    listaCarritos[indexCarrito].codSucursal = '';
//    //    LimpiarTextBoxProductosBuscados();
//    CerrarContenedorTransfer();
//}
